...
def rescaleFrame(image,scale=0.20):
...
def warpImage(img):
    w = img.shape[1]
    h = img.shape[0]
    points = createPoints(w,h,0.2,0.7) #adjust parameters
...
image = cv.imread('images/img2.jpg')

warpedImage = warpImage(image)
image_scaled = rescaleFrame(warpedImage)
