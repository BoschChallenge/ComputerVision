...
class LineDetect:
    ...
    def calculateAngle(self,lineA,lineB):
    ...
    def canny_func(self,image):
        ...
        edg = self.region_of_interest(edges)
        return edg
    ...
    def detectLines(self,frame):
        img = self.warpImage(frame)
        edges = self.canny_func(img)
        lines = cv2.HoughLinesP(edges, 1, np.pi / 180, 150, np.array([]), minLineLength=40, maxLineGap=5)
        averaged_lines = self.average_slope_intercept(img,lines)
        ...
        if averaged_lines is not None:
            ...
            if len(averaged_lines) != 2:
                if right_exist:
                    ...
                    x,y = self.line_intersection(([x1,y1],[x2,y2]),([rx1,ry1],[rx2,ry2]))
                    if(x != 0 and y != 0):
